...
func (cfg *ServerConfig) checkValid() bool {
	strRemoteID := fmt.Sprintf("%d%d", cfg.ServerID, cfg.ZoneID)
	...
	cfg.NodeName = fmt.Sprintf("%s_%s_%d", cfg.NodeType, cfg.SelfIP, cfg.ZoneID)
...
func (cfg *ServerConfig) PrintServerConfig() {
	...
	xlog.Debugf("NodeType=%s, NodeName=%s, ServerID=%d, ZoneID=%d, RemoteID=%d",
		cfg.NodeType, cfg.NodeName, cfg.ServerID, cfg.ZoneID, cfg.RemoteID)
